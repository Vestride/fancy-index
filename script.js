{
    ...
    /**
     * Adds a button to toggle the color mode
     * @return {null}
     */
    function addColorModeToggle() {
        const colorModeToggleBtnSvg = `<svg aria-hidden="true"><title>Toggle Color Mode</title><use href="#toggle"></use></svg>`;

        colorModeBtn = document.createElement('button');
        colorModeBtn.classList = 'color-mode-btn';
        // Add event listener to colorModeBtn to toggle the color mode
        colorModeBtn.addEventListener("click", e => {
            setColorMode(getColorMode(true));
        });

        document.getElementById('page-header').appendChild(colorModeBtn).innerHTML = colorModeToggleBtnSvg;
    }
    ...
    if (window.CSS && CSS.supports("color", "var(--primary)")) {
         ...
         * @param {boolean} inverted If set to to true it returns the opposite
         * @return {string} light or dark
         */
        function getColorMode(inverted = false) {
        ...
        // Add event listener for changes on the preferred color theme
        window.matchMedia('(prefers-color-scheme: light)')
            .addEventListener('change', e => {
                setColorMode(e.matches ? "light" : "dark");
            });
    } else {
        // If the feature isn't supported, hide the toggle button
